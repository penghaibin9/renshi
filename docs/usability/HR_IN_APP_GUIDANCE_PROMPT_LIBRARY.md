...
- 全模块通用低学习成本问法：16 条；
- 每个模块页面至少显示 23 个可点击问法（原有业务问法 4 + 模块少填问法 3 + 通用问法 16）；
- 新增“提交前核对、已有值不重填、哪些必须人工确认、只改一项最短路径、字段为什么要填、提交后谁处理”等高频问法。
...
“复制脱敏提问”只携带模块、页面类型、用户主动选择的问题、办理说明，以及“必填完成度/缺失字段名称”这类无值元数据；不从页面抓取姓名、身份证号、联系方式、工资、合同正文、附件正文或人员 ID，也不发送任何表单字段值。AI 可以解释流程、材料、前置条件、状态和下一步，但不能替学校作录用、审批、资格认定、考核、聘任、薪酬或离退正式决定。
