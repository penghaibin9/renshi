...
class Hr16AccessContractTests(SimpleTestCase):
    def setUp(self):
        self.request = RequestFactory().get("/api/v1/hr/exit/dashboard/")

    @patch("hr_exit.api.resolve_tenant_from_request", return_value=7)
    @patch("hr_exit.api.get_allowed_company_ids", return_value=set())
    def test_empty_membership_fails_closed(self, _allowed, _tenant):
        self.request.user = UserStub({READ_PERMISSION})
        with self.assertRaises(HrExitAccessError) as ctx:
