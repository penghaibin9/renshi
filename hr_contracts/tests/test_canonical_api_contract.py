...
class Hr07RegistryContractTests(SimpleTestCase):
    def test_canonical_routes_are_registered_without_recovery_only_modules(self):
        ...
        self.assertLessEqual(expected, routes)

    def test_permissions_use_global_registry(self):
        for key in (
            PERM_AGREEMENT_VIEW,
            PERM_AGREEMENT_CREATE,
            PERM_AGREEMENT_SIGN,
    ...
    def test_events_use_global_registry(self):
        for event_name in (
            EVENT_AGREEMENT_CREATED,
            EVENT_AGREEMENT_SIGNED,
            EVENT_AGREEMENT_EFFECTIVE,
            ...
            self.assertEqual(definition.module_code, "HR07")
            self.assertEqual(definition.aggregate, "agreement")
