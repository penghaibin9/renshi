...
class ContractLifecycleServiceTests(TestCase):
    ...
    def test_renewal_signs_append_only_successor_without_overwriting_previous(self, version_objects):
        case = MagicMock()
        case.id = "case-1"
        ...
        ), patch.object(self.service, "_current_effective_version", return_value=previous):
        ...
        self.assertEqual(agreement.status, HrContractAgreement.Status.SIGNED_WAITING_EFFECTIVE)
    ...
    def test_future_successor_cannot_activate_early(self):
        ...
        with patch.object(self.service, "_case", return_value=case), patch.object(
            self.service, "_agreement", return_value=agreement
        ), patch("hr_contracts.services.lifecycle_service.HrContractVersion.objects") as objects:
            objects.select_for_update.return_value.filter.return_value = qs
            with self.assertRaises(ContractServiceError) as cm:
                self.service.activate_successor_version(
                    case_id="case-1", version_id="version-2", as_of=date(2026, 12, 31)
                )
    ...
    def test_activation_supersedes_previous_then_publishes_successor(self):
        ...
        with patch.object(self.service, "_case", return_value=case), patch.object(
            ...
            objects.select_for_update.return_value.filter.side_effect = [next_qs, prev_qs]
            result = self.service.activate_successor_version(
                case_id="case-1", version_id="version-2", as_of=date(2027, 1, 1)
            )
    ...
    def test_termination_requires_approved_case_and_effective_date(self):
        ...
        with patch.object(self.service, "_case", return_value=case), patch.object(
            self.service, "_agreement", return_value=agreement
        ), patch.object(self.service, "_current_effective_version", return_value=current):
            with self.assertRaises(ContractServiceError) as cm:
                self.service.effect_termination(case_id="case-1", as_of=date(2026, 9, 30))
            self.assertEqual(cm.exception.code, "CONTRACT_NOT_EFFECTIVE_YET")
            current.save.assert_not_called()

            result = self.service.effect_termination(case_id="case-1", as_of=date(2026, 10, 1))
