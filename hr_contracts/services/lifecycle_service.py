...
class ContractLifecycleService:
    ...
    def _case(self, case_id):
        ...
        if item is None:
            raise ContractServiceError("CONTRACT_CASE_NOT_FOUND", "contract case not found inside tenant")
        return item

    def _current_effective_version(self, agreement):
        if not agreement.current_version_no:
            raise ContractServiceError("CONTRACT_FORMAL_VERSION_REQUIRED", "formal contract version is required")
        item = (
        ...
        if item is None:
            raise ContractServiceError("CONTRACT_VERSION_NOT_FOUND", "current contract version not found")
        return item
    ...
    def create_case(
        ...
        *,
        ...
        if case_type not in {
            ...
            raise ContractServiceError("CONTRACT_CASE_TYPE_INVALID", "only RENEW/CHANGE/TERMINATE are supported")
        ...
        if requested_effective_from is None:
            raise ContractServiceError("CONTRACT_EFFECTIVE_DATE_REQUIRED", "requested effective date is required")
        if requested_effective_to is not None and requested_effective_to <= requested_effective_from:
            raise ContractServiceError("CONTRACT_EFFECTIVE_RANGE_INVALID", "effective_to must be later than effective_from")
        ...
        existing = (
            HrContractCase.objects.select_for_update()
            .filter(tenant_id=self.tenant_id, case_no=case_no.strip())
            .first()
        )
    ...
    def submit_case(self, *, case_id) -> HrContractCase:
        ...
        if item.status == HrContractCase.Status.SUBMITTED:
            return item
        if item.status not in {HrContractCase.Status.DRAFT, HrContractCase.Status.RETURNED}:
            raise ContractServiceError("CONTRACT_CASE_INVALID_STATE", f"case status {item.status} cannot be submitted")
    ...
    def approve_case(self, *, case_id) -> HrContractCase:
        ...
        if item.status == HrContractCase.Status.APPROVED:
            return item
        if item.status != HrContractCase.Status.SUBMITTED:
            raise ContractServiceError("CONTRACT_CASE_INVALID_STATE", f"case status {item.status} cannot be approved")
        item.status = HrContractCase.Status.APPROVED
        item.approved_at = timezone.now()
        item.approved_by = self.actor_user_id
        item.updated_by = self.actor_user_id
        item.save(update_fields=["status", "approved_at", "approved_by", "updated_by", "updated_at"])
        return item
    ...
    def sign_successor_version(
        ...
        *,
        ...
        if case.case_type not in {HrContractCase.CaseType.RENEW, HrContractCase.CaseType.CHANGE}:
            raise ContractServiceError("CONTRACT_CASE_TYPE_INVALID", "only renewal/change can create successor versions")
        ...
        if case.status != HrContractCase.Status.APPROVED:
            raise ContractServiceError("CONTRACT_CASE_INVALID_STATE", "case must be approved before signing")
        if not signed_document_ref.strip():
            raise ContractServiceError("CONTRACT_SIGNED_DOCUMENT_REQUIRED", "signed document reference is required")
        if not content_snapshot:
            raise ContractServiceError("CONTRACT_CONTENT_REQUIRED", "signed content snapshot cannot be empty")
        ...
        if previous.status != HrContractVersion.Status.EFFECTIVE:
            raise ContractServiceError("CONTRACT_VERSION_INVALID_STATE", "current formal version must be EFFECTIVE")
    ...
    @transaction.atomic
    def activate_successor_version(self, *, case_id, version_id, as_of: Optional[date] = None) -> HrContractVersion:
        effective_day = as_of or timezone.localdate()
        case = self._case(case_id)
        if case.status == HrContractCase.Status.EFFECTIVE:
            existing = HrContractVersion.objects.filter(id=version_id, tenant_id=self.tenant_id).first()
            if existing is not None and existing.status == HrContractVersion.Status.EFFECTIVE:
                return existing
        if case.status != HrContractCase.Status.EFFECT_PENDING:
            raise ContractServiceError("CONTRACT_CASE_INVALID_STATE", "case is not waiting for effect")

        agreement = self._agreement(case.agreement_id)
        version = (
            HrContractVersion.objects.select_for_update()
            .filter(id=version_id, tenant_id=self.tenant_id, agreement_id=agreement.id)
            .first()
        )
        if version is None:
            raise ContractServiceError("CONTRACT_VERSION_NOT_FOUND", "successor version not found")
        if version.version_no != agreement.current_version_no or version.status != HrContractVersion.Status.SIGNED:
            raise ContractServiceError("CONTRACT_VERSION_CONFLICT", "version is not the current signed successor")
        if version.effective_from > effective_day:
            raise ContractServiceError("CONTRACT_NOT_EFFECTIVE_YET", "successor effective date has not arrived")
        if not version.supersedes_version_id:
            raise ContractServiceError("CONTRACT_SUPERSEDED_VERSION_REQUIRED", "successor does not reference previous version")
        ...
        if previous is None or previous.status != HrContractVersion.Status.EFFECTIVE:
            raise ContractServiceError("CONTRACT_PREVIOUS_VERSION_CONFLICT", "previous effective version is missing or stale")
        ...
        previous.save(update_fields=["effective_to", "status", "updated_by", "updated_at"])
        ...
        case.updated_by = self.actor_user_id
        case.save(update_fields=["status", "effect_receipt_json", "last_effect_error", "updated_by", "updated_at"])
        return version

    @transaction.atomic
    def effect_termination(self, *, case_id, as_of: Optional[date] = None) -> HrContractCase:
        effective_day = as_of or timezone.localdate()
        case = self._case(case_id)
        if case.case_type != HrContractCase.CaseType.TERMINATE:
            raise ContractServiceError("CONTRACT_CASE_TYPE_INVALID", "case is not a termination")
        agreement = self._agreement(case.agreement_id)
        if case.status == HrContractCase.Status.EFFECTIVE and agreement.status == HrContractAgreement.Status.TERMINATED:
            return case
        if case.status != HrContractCase.Status.APPROVED:
            raise ContractServiceError("CONTRACT_CASE_INVALID_STATE", "termination must be approved before effect")
        terminate_on = case.requested_effective_from
        if terminate_on is None:
            raise ContractServiceError("CONTRACT_EFFECTIVE_DATE_REQUIRED", "termination effective date is required")
        if terminate_on > effective_day:
            raise ContractServiceError("CONTRACT_NOT_EFFECTIVE_YET", "termination effective date has not arrived")
        ...
        if terminate_on <= current.effective_from:
            raise ContractServiceError("CONTRACT_TERMINATION_DATE_INVALID", "termination date must be after version effective date")
        ...
        current.save(update_fields=["effective_to", "status", "updated_by", "updated_at"])
