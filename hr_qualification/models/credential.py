...
from hr_qualification.crypto import encrypt_certificate_no
...
class HrPersonCredential(models.Model):
    ...
    external_engagement_id = models.BigIntegerField(null=True, blank=True, db_index=True)
    ...
    def save(self, *args, **kwargs):
        """Fail-safe write boundary: plain certificate bytes never reach the DB."""
        if self.certificate_no_cipher:
            self.certificate_no_cipher = encrypt_certificate_no(
                self.tenant_id,
                bytes(self.certificate_no_cipher),
            )
