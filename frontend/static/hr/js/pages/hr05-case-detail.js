...
(function () {
  ...
  async function command(caseId, action, button) {
    ...
    try {
      await window.HrApi.request("/api/v1/hr/onboarding/cases/" + encodeURIComponent(caseId) + "/" + action, {method:"POST"});
      await load();
    } catch (err) {
  ...
  async function load() {
    ...
    try {
      const res = await window.HrApi.request("/api/v1/hr/onboarding/cases/" + encodeURIComponent(caseId)); const item = res.data?.data || {};
