...
def dashboard(request):
    ...
    except HrAppointmentAccessError as exc: return JsonResponse({"error":{"code":"ACCESS_DENIED","message":str(exc)}}, status=403)
    data = dashboard_snapshot(tenant_id)
    data.update({"apiVersion":"1.0","schemaVersion":"hr14.workspace.1","generatedAt":timezone.now().isoformat()})
