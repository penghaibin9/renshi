...
(function () {
  ...
  async function load(caseId) {
    ...
    host.innerHTML = stateHtml("正在读取协同任务", "等待当前 case 的 canonical task API。", false);
    try {
      ...
      if (!items.length) { host.innerHTML = stateHtml("当前 case 暂无协同任务", "API 已成功返回空任务清单。", false); return; }
      host.innerHTML = '<table class="hr-table"><thead><tr><th>任务</th><th>类别</th><th>责任角色</th><th>阻塞级别</th><th>状态</th><th>截止</th></tr></thead><tbody>' + items.map(function (item) { return '<tr><td>' + escapeHtml(item.title || item.code || "—") + '</td><td>' + escapeHtml(item.category || "—") + '</td><td>' + escapeHtml(item.responsibleRoleLabel || item.responsible_role || "—") + '</td><td>' + escapeHtml(item.blockingLevelLabel || item.blocking_level || "—") + '</td><td><span class="hr05-badge hr05-badge--' + safeStatusClass(item.status) + '">' + escapeHtml(item.statusLabel || item.status || "—") + '</span></td><td>' + escapeHtml(item.due_at || "—") + '</td></tr>'; }).join("") + '</tbody></table>';
