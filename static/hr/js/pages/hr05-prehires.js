...
(function () {
  ...
  async function load(keyword) {
    ...
    if (!host) return;
    host.innerHTML = stateHtml("正在读取待报到人员", keyword ? "按当前关键词查询。" : "读取当前学校可见入职 case。", false);
    try {
      const res = await window.HrApi.request("/api/hr/v1/onboarding/cases", { params: { keyword: keyword || "", page: 1, pageSize: 100 } });
      const payload = res.data?.data || {};
      const items = payload.items || [];
      if (!items.length) { host.innerHTML = stateHtml("暂无匹配的待报到人员", keyword ? "当前搜索条件没有返回结果。" : "当前学校暂无可见入职 case。", false); return; }
      host.innerHTML = '<table class="hr-table"><thead><tr><th>入职单</th><th>来源</th><th>人员类别</th><th>预计报到</th><th>状态</th><th>人员匹配</th><th>进入</th></tr></thead><tbody>' + items.map(function (item) {
        ...
        return '<tr><td>' + escapeHtml(item.case_no || "—") + '</td><td>' + escapeHtml(item.sourceTypeLabel || item.source_type || "—") + '</td><td>' + escapeHtml(item.staffCategoryLabel || item.staff_category || "—") + '</td><td>' + escapeHtml(item.expected_report_date || "—") + '</td><td><span class="hr05-badge hr05-badge--' + safeStatusClass(item.status) + '">' + escapeHtml(item.statusLabel || item.status || "—") + '</span></td><td>' + escapeHtml(item.personMatchStatusLabel || item.person_match_status || "—") + '</td><td><div class="hr05-actions"><a href="/hr/onboarding/prehires/' + id + '">详情</a><a href="/hr/onboarding/reporting/' + id + '">报到</a><a href="/hr/onboarding/materials?case_id=' + id + '">材料</a></div></td></tr>';
      }).join("") + '</tbody></table>';
    } catch (err) {
