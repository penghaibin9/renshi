...
(function () {
  ...
  async function load(caseId) {
    ...
    try {
      ...
      host.innerHTML = '<table class="hr-table"><thead><tr><th>材料</th><th>阻塞阶段</th><th>必需</th><th>状态</th><th>有效期</th></tr></thead><tbody>' + items.map(function (item) { return '<tr><td>' + escapeHtml(item.label || "未命名材料") + '</td><td>' + escapeHtml(item.blockingPhaseLabel || "阻塞阶段待确认") + '</td><td>' + (item.required ? "是" : "否") + '</td><td><span class="hr05-badge hr05-badge--' + safeStatusClass(item.status) + '">' + escapeHtml(window.HrApi.statusLabel(item.status, item.statusLabel)) + '</span></td><td>' + escapeHtml(item.expiry_date || "—") + '</td></tr>'; }).join("") + '</tbody></table>';
