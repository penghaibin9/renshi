...
def main():
    ...
    with sync_playwright() as p:
        ...
        page=context.new_page();page.set_default_timeout(6000);errors=[];page.on('pageerror',lambda error:errors.append(str(error)))
        page.on('dialog',lambda dialog:dialog.accept() if state['dialog']=='accept' else dialog.dismiss())
        page.set_content(admin_html)
        ...
        page.set_content(admin_html);expect(page.locator('[data-progress-state="NEEDS_STAFF"]').count()==1,'rerender fabricated completion')
        ...
        checks.extend(['server_state_rendered','memo_cannot_complete_server_steps','rerender_retains_provided_server_state','optional_profile_collapsed','mobile_no_overflow'])
        import base64
        fixture={'preview':preview,'completed':completed,'template':base64.b64encode((ROOT/'backend/hr_staff/resources/staff_import_template.xlsx').read_bytes()).decode()}
        adapter = "<script>window.__fixture="+json.dumps(fixture)+";"+r"""
          Object.defineProperty(document,"cookie",{configurable:true,get:()=>""}); // opaque-origin fixture only
          window.__counts={commit_posts:0,status_gets:0,uploads:0}; window.__committed=false;
          window.fetch=async function(url,opts={}) {
            const payload=data=>new Response(JSON.stringify(data),{status:200,headers:{'Content-Type':'application/json'}});
            if(url.endsWith('/template')) return new Response(Uint8Array.from(atob(__fixture.template),c=>c.charCodeAt(0)),{headers:{'Content-Type':'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}});
            if(url.endsWith('/errors')) return new Response('原文件行号,字段,错误原因\n3,所属部门,本校未找到部门\n',{headers:{'Content-Type':'text/csv'}});
            if(url.endsWith('/commit')) {__counts.commit_posts++;__committed=true;throw new TypeError('Synthetic connection loss');}
            if(url.endsWith('/import') && opts.method==='POST'){__counts.uploads++;return payload({data:__fixture.preview});}
            if(url.includes('/import/')){__counts.status_gets++;return payload({data:__committed?__fixture.completed:__fixture.preview});}
            if(url.startsWith('/api/hr/v1/staff?'))return payload({items:[],total:0,page:1,dataBasis:'TEST_FIXTURE',asOf:'2026-09-17'});
            return new Response('Unsupported fixture request',{status:404});
          };
        """+"</script>"
        page.set_viewport_size({'width':1440,'height':1100});page.set_content(staff_html.replace('<script>'+script,adapter+'<script>'+script))
        ...
        expect(page.evaluate('window.__counts.commit_posts')==0,'upload auto-committed')
        page.screenshot(path=str(ART/'import-preview.png'),full_page=True)
        page.locator('#importCommit').click();expect(page.evaluate('window.__counts.commit_posts')==0,'cancel did not cancel')
        ...
        expect(not page.locator('#importRefresh').is_disabled(),'lost in-page job after timeout')
        page.screenshot(path=str(ART/'import-connection-loss.png'),full_page=True)
        # A stale lease is a distinct server-authorised action, not a blind retry.
        page.evaluate("__fixture.completed.status='COMMITTING';__fixture.completed.resumeAllowed=true;__fixture.completed.remainingRows=1")
        page.locator('#importRefresh').click();page.wait_for_function('document.getElementById("importCommit").textContent.includes("继续未完成行")')
        expect(not page.locator('#importCommit').is_disabled(),'stale lease has no recovery action')
        state['dialog']='dismiss';page.locator('#importCommit').click()
        expect(page.evaluate('window.__counts.commit_posts')==1,'cancelled resume still committed')
        page.evaluate("__fixture.completed.status='PARTIAL_FAILED';__fixture.completed.resumeAllowed=false")
        ...
        expect(page.evaluate('window.__counts.commit_posts')==1,'refresh repeated POST')
        ...
        checks.extend(['template_download_actual_bytes','no_commit_before_preview','preview_escaped_errors','confirmation_cancel','lost_response_blocks_blind_retry','in_page_job_retained','GET_recovers_result_without_POST','error_download','file_change_invalidates_preview','server_authorised_stale_resume','cancel_resume_no_extra_POST'])
        expect(not errors,'JavaScript errors: '+str(errors))
        counters=page.evaluate('window.__counts')
        browser.close()
    report={'status':'PASS','checks':checks,'count':len(checks),'counters':counters,'page_errors':errors,
        ...
        'execution_mode':'page.set_content; window.fetch simulated; real native DOM events',
        'http_navigation':'BLOCKED_BY_BROWSER_POLICY', 'full_page_reload_and_storage_persistence':'NOT_RUN',
        'fixture_storage':'none; production try/catch handles unavailable opaque-origin storage'}
