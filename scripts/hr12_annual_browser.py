...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            page.locator("#hr12-policy-code").fill(
                "HR12-BROWSER-GOVERNANCE"
            )
            page.locator("#hr12-policy-name").fill(
                "HR12 浏览器制度治理"
            )
            ...
            # Production HR12 no longer accepts a manually selected grade.
            # The formal grade must be derived from submitted reviewer facts and
            # the published policy, so the browser proves that no override field
            # exists before executing the real finalization button.
            require(
            ...
            finalize_api = (
                f"/api/v1/hr/assessments/cases/{case_id}/finalize"
            )
            with page.expect_response(
            ...
            try:
                context.tracing.stop(
                    path=str(ARTIFACT_DIR / "trace.zip")
                )
            finally:
                context.close()
                browser.close()
