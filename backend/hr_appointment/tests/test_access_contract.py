...
from hr_appointment.api import HrAppointmentAccessError, READ_PERMISSION, resolve_request_tenant
...
class Hr14AccessContractTests(SimpleTestCase):
    def setUp(self):
        self.request = RequestFactory().get("/api/v1/hr/appointments/dashboard/")

    @patch("hr_appointment.api.resolve_tenant_from_request", return_value=7)
    @patch("hr_appointment.api.get_allowed_company_ids", return_value=set())
