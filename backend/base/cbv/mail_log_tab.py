"""
This page is handling the cbv methods of mail log tab in employee individual page.
"""

from typing import Any

from django.contrib import messages
from django.urls import reverse
from django.utils.decorators import method_decorator
from django.utils.translation import gettext_lazy as _

from accessibility.cbv_decorators import enter_if_accessible
from base.email_logging import email_log_recipient_q, resolve_email_log_company
from base.filters import MailLogFilter
from base.models import EmailLog
from employee.models import Employee
from horilla.http.response import HorillaRedirect
from horilla_views.cbv_methods import login_required
from horilla_views.generic.cbv.views import HorillaDetailedView, HorillaListView


def _check_reporting_manager(request, *args, **kwargs):
    employee_id = kwargs.get("pk")
    manager = request.user.employee_get
    return manager.reporting_manager.filter(employee_id_id=employee_id).exists()


def _check_mail_log_reporting_manager(request, *args, **kwargs):
    manager = request.user.employee_get
    direct_reports = manager.reporting_manager.all()
    addresses = list(direct_reports.values_list("employee_id__email", flat=True))
    addresses.extend(direct_reports.values_list("email", flat=True))
    company = resolve_email_log_company(request)
    return (
        EmailLog.objects.filter(pk=kwargs.get("pk"), company_id=company)
        .filter(email_log_recipient_q(*addresses))
        .exists()
    )


@method_decorator(login_required, name="dispatch")
@method_decorator(
    enter_if_accessible(
        feature="view_mail_log",
        perm="employee.view_employee",
        method=_check_reporting_manager,
    ),
    name="dispatch",
)
class MailLogTabList(HorillaListView):
    """
    list view for mail log  tab
    """

    model = EmailLog
    filter_class = MailLogFilter

    def __init__(self, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        self.view_id = "maillog"

        pk = self.request.resolver_match.kwargs.get("pk")
        self.search_url = reverse("individual-email-log-list", kwargs={"pk": pk})

    # def get_context_data(self, **kwargs: Any):
    #     context = super().get_context_data(**kwargs)
    #     pk = self.kwargs.get('pk')
    #     context["search_url"] = f"{reverse('individual-email-log-list',kwargs={'pk': pk})}"
    #     return context

    def dispatch(self, request, *args, **kwargs):
        pk = kwargs.get("pk")
        if not Employee.objects.filter(id=pk).exists():
            messages.error(request, _("Employee not found."))
            return HorillaRedirect(request)
        return super().dispatch(request, *args, **kwargs)

    def get_queryset(self):
        queryset = super().get_queryset()
        pk = self.kwargs.get("pk")
        employee = Employee.objects.get(id=pk)
        addresses = [employee.email]
        if employee.employee_work_info and employee.employee_work_info.email:
            addresses.append(employee.employee_work_info.email)
        query_filter = email_log_recipient_q(*addresses)
        return queryset.filter(
            query_filter, company_id=employee.get_company()
        ).order_by("-created_at")

    columns = [
        (_("Subject"), "subject"),
        (_("Date"), "created_at"),
        (_("Status"), "status_display"),
    ]

    sortby_mapping = [
        (_("Subject"), "subject"),
        (_("Date"), "created_at"),
    ]

    row_attrs = """
                hx-get='{mail_log_detail_view}?instance_ids={ordered_ids}'
                hx-target="#genericModalBody"
                data-target="#genericModal"
                data-toggle="oh-modal-toggle"
                """


@method_decorator(login_required, name="dispatch")
@method_decorator(
    enter_if_accessible(
        feature="view_mail_log",
        perm="employee.view_employee",
        method=_check_mail_log_reporting_manager,
    ),
    name="dispatch",
)
class MailLogDetailView(HorillaDetailedView):
    """
    detail view for mail log tab
    """

    template_name = "cbv/mail_log_tab/iframe.html"
    model = EmailLog

    def get_queryset(self):
        queryset = super().get_queryset()
        selected = self.request.session.get("selected_company")
        if selected == "all":
            allowed = getattr(self.request, "all_my_company_ids", None) or ()
            return queryset.filter(company_id_id__in=allowed)
        company = resolve_email_log_company(self.request)
        return queryset.filter(company_id=company) if company else queryset.none()

    def get_context_data(self, **kwargs: Any):
        context = super().get_context_data(**kwargs)
        context["log"] = context.get("object")
        return context

    header = {"title": "", "subtitle": "", "avatar": ""}
