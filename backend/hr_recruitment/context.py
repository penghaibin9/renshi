"""
hr_recruitment/context.py

Hr04RequestContext —— HR04 所有 selector/service/api 共享的请求上下文。

硬合同（总册 1.1 A0 + HR01 context 继承）：
- tenant/school 由服务端可信上下文解析，禁止信任前端 tenant_id。
- 无学校上下文 → fail-closed（403 TENANT_CONTEXT_REQUIRED），不做 isnull=True 全校兜底。
- 所有"今天/本月/本年"判断基于 school_timezone，禁止 date.today() 直用。
- authority_mode 显式：LEGACY_RECRUITING_ONLY / DUAL_READ_COMPARE / HR04_AUTHORITY。
"""
...
class Hr04RequestContext:
    """
    HR04 请求上下文（冻结）。

    字段与 HR01 HrRequestContext 对齐，另加 authority_mode 默认
    LEGACY_RECRUITING_ONLY（Authority 切换前）。
    """
    ...
    authority_mode: str = "LEGACY_RECRUITING_ONLY"
...
def build_hr04_context(
    *,
    ...
    """从请求参数构造 Hr04RequestContext（服务端必须重新验证 scope，不信任前端参数）。"""
