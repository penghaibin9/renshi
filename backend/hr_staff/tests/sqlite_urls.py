"""Canonical HR03 browser and API routes for isolated tests."""

from django.urls import include, path, re_path

from horilla.legacy_hr_api import legacy_hr_api_redirect
from hr_staff.api.urls import urlpatterns as legacy_api_patterns
...
def _canonical_pattern(pattern):
    ...
    if not route.startswith(legacy_prefix):
        raise RuntimeError(f"unexpected HR03 legacy API route: {route}")
    return path(
