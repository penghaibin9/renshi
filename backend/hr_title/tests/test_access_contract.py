...
from hr_title.api import HrTitleAccessError, READ_PERMISSION, resolve_request_tenant
...
class Hr13AccessContractTests(SimpleTestCase):
    def setUp(self):
        self.request = RequestFactory().get("/api/v1/hr/titles/dashboard/")

    @patch("hr_title.api.resolve_tenant_from_request", return_value=7)
    @patch("hr_title.api.get_allowed_company_ids", return_value=set())
    def test_empty_membership_fails_closed(self, _allowed, _tenant):
        self.request.user = UserStub({READ_PERMISSION})
        with self.assertRaises(HrTitleAccessError) as ctx:
