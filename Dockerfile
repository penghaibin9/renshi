...
COPY --chown=appuser:appuser deploy/docker/entrypoint.sh /entrypoint.sh
...
RUN mkdir -p .runtime/staticfiles .runtime/media \
...
CMD ["gunicorn", "horilla.wsgi:application", "--config", "deploy/docker/gunicorn.conf.py"]
