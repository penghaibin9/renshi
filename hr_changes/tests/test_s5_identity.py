...
from hr_changes.models import HrChangeAction, HrPersonnelChangeCase
...
class IdentityChangeServiceTests(TestCase):
    ...
    def _create(self, action_code, proposals, **kw):
    ...
    def test_post_category_change(self):
        case = self._create(
            ...
            [{"domain": "assignment", "field_code": "post_catalog", "proposed_value_display": "管理岗"}],
        )
    ...
    def test_employee_category_change(self):
        case = self._create(
            ...
            [{"domain": "staff", "field_code": "staff_category_code", "proposed_value_display": "行政管理"}],
        )
        self.assertEqual(case.action_id.code, ChangeActionCode.EMPLOYEE_CATEGORY_CHANGE)

    def test_disallowed_field_rejected(self):
        # EMPLOYEE_CATEGORY_CHANGE 不允许改 position
        with self.assertRaises(ChangeServiceError) as cm:
            self._create(
                ChangeActionCode.EMPLOYEE_CATEGORY_CHANGE,
                [{"domain": "assignment", "field_code": "position", "proposed_value_display": "P1"}],
            )
    ...
    def test_add_secondary_requires_org_position(self):
        ...
        with self.assertRaises(ChangeServiceError) as cm:
            self._create(
                ...
                [{"domain": "assignment", "field_code": "organization", "proposed_value_ref": str(org.id)}],
            )
        self.assertEqual(cm.exception.code, "CHANGE_INVALID_PAYLOAD")

    def test_employment_type_hr07_policy_warning(self):
        action = make_action(TENANT, ChangeActionCode.EMPLOYMENT_TYPE_CHANGE)
        action.followup_policy_json = {"employment_type_policy": "REQUIRE_HR07_CONTRACT"}
        action.save()
        case = self._create(
            ChangeActionCode.EMPLOYMENT_TYPE_CHANGE,
            [{"domain": "relationship", "field_code": "relationship_type", "proposed_value_display": "合同制"}],
        )
    ...
    def test_change_matrix(self):
        ...
        dims = {m["dimension"]: m for m in matrix}
        self.assertEqual(dims["人员类别"]["after"], "行政管理")
        self.assertTrue(dims["人员类别"]["affectsDownstream"])
        self.assertFalse(dims["岗位类别"]["affectsDownstream"])

    def test_validate_required_fields(self):
        # EMPLOYMENT_TYPE_CHANGE 缺少 relationship_type
        case = self._create(
            ChangeActionCode.EMPLOYMENT_TYPE_CHANGE,
            [{"domain": "relationship", "field_code": "employment_type", "proposed_value_display": "全职"}],
        )
        result = IdentityChangeService(TENANT).validate_identity_change(case)
        self.assertTrue(any(b["code"] == "CHANGE_INVALID_PAYLOAD" for b in result["blockers"]))
...
class IdentityApiTests(TestCase):
    ...
    def test_create_identity_api(self):
        ...
        with mock.patch("hr_changes.api.identity_changes.make_hr_change_context", return_value=ctx()):
            resp = identity_api.identity_change_create(self._req("post", "/api/hr/v1/changes/identity-changes", body))
    ...
    def test_identity_list_api(self):
        ...
        with mock.patch("hr_changes.api.identity_changes.make_hr_change_context", return_value=ctx()):
            resp = identity_api.identity_change_list(self._req("get", "/api/hr/v1/changes/identity-changes"))
