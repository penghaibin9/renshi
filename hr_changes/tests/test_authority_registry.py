...
from hr_changes.authority_registry import CANONICAL_PERMISSION_KEYS, EVENT_DEFINITIONS


class Hr06AuthorityRegistryTests(SimpleTestCase):
    def test_hr06_owns_every_registered_permission(self):
        self.assertTrue(CANONICAL_PERMISSION_KEYS)
        for key in CANONICAL_PERMISSION_KEYS:
            definition = permission_registry.get(key)
            self.assertEqual(definition.module_code, "HR06")
            self.assertTrue(key.startswith("hr.change."))

    def test_hr06_owns_every_registered_event(self):
        ...
        for expected in EVENT_DEFINITIONS:
            ...
            self.assertEqual(actual.module_code, "HR06")
