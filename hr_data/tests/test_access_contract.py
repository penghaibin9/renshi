...
from hr_data.api import HrDataAccessError, READ_PERMISSION, resolve_request_tenant
...
class Hr18AccessContractTests(SimpleTestCase):
    def setUp(self):
        self.request = RequestFactory().get("/api/v1/hr/data/dashboard/")

    @patch("hr_data.api.resolve_tenant_from_request", return_value=7)
    @patch("hr_data.api.get_allowed_company_ids", return_value=set())
    def test_empty_membership_fails_closed(self, _allowed, _tenant):
        self.request.user = UserStub({READ_PERMISSION})
        with self.assertRaises(HrDataAccessError) as ctx:
